...
export default class ModuleBuilder {
  ...
  _func (name, params=[], results=[], locals=[], body=[], exported = false, imported = false) {
  ...
  build ({ metrics = true } = {}) {
    ...
    if (this.types.length) { bytes.write(section.type(
      this.types.map(type =>
    ...
    if (this.funcs.length) { bytes.write(section.function(
      this.funcs.map(func =>
        func.type_idx
      ))) }
    ...
    if (this.funcs.length) { bytes.write(section.code(
      this.funcs.map(func =>
