...
export default class ModuleBuilder {
  ...
  getGlobalIndexOf (name) {
    return this.globs.find(glob => glob.name === name).idx
  }
  ...
  global (name, mut, valtype, expr) {
    const global_idx = this.globs.length
    this.globs.push({ idx: global_idx, name, valtype, mut, expr })
    return this
  }
  ...
  build () {
    ...
    if (this.globs.length) { bytes.write(section.global(
      this.globs.map(glob => [glob.mut, glob.valtype, glob.expr]))) }
