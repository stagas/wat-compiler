...
export default class ModuleBuilder {
  ...
  get exports () { return this.codes.filter(func => func.exported) }
  get funcs () { return this.codes.filter(func => !func.imported) }

  getTypeIndexOf (params, results) {
  ...
  import (type, mod, field, params, results) {
    if (type === 'func') {
      const type_idx = this.getTypeIndexOf(params, results)
      this.imports.push({ mod, field, type, desc: [type_idx] })
      const func_idx = this.codes.length
      this.codes.push({ idx: func_idx, name: mod+'.'+field, type_idx, imported: true })
    }
    return this
  }
  ...
  func (name, params, results, locals, body, exported = false) {
    const type_idx = this.getTypeIndexOf(params, results)
    const func_idx = this.codes.length
  ...
  build () {
    ...
    if (this.exports.length) { bytes.write(section.export(
      this.exports.map(func =>
        [ 'func', func.idx, func.name ] // TODO: refactor to match import & handle other types
      ))) }
