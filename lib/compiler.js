...
export default function compile (node) {
  ...
  function cast (param, context) {
    switch (param.kind) {
      case 'number': return parseFloat(param.value)
      case 'hex': return parseInt(param.value)
  ...
  function evaluate (node, context = g) {
    ...
    switch (instr) {
      ...
      case 'i32.load16_u':
        if (instr === 'i32.load16_u') address.align = 0
      case 'i32.load8_s':
        if (instr === 'i32.load8_s') address.align = 4
      case 'i32.load8_u': {
        if (instr === 'i32.load8_u') address.align = 0
        for (const p of node.params) {
          address[p.param.value] = cast(p.value)
        }
        const args = [address.align/2, address.offset]
        const expr = node.children.flatMap(x => evaluate(x, context))
        return bytes(instr, args, expr)
      }
