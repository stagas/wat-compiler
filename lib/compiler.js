...
class GlobalContext {
  ...
  lookup (name, instr) {
    ...
    if (index === -1) console.log(name, instr)
    return index
  }
}
...
export default function compile (node) {
  ...
  function cast (param, context, instr) {
    switch (param.kind) {
  ...
  class FunctionContext {
    ...
    lookup (name, instr) {
      ...
      if (!~index) index = g.lookup(name, instr)
  ...
  function evaluate (node, context = g) {
    ...
    switch (instr) {
      ...
      case 'i64.store':
        if (instr === 'i64.store') address.align = 6
      case 'i32.store':
      ...
      case 'i64.load':
        if (instr === 'i64.load') address.align = 6
      case 'i32.load':
      ...
      case 'br_table': {
        ...
        const args = node.params.map(x => cast(x.param, context, node.instr.value))
        const expr = node.children.flatMap(x => evaluate(x, context))
