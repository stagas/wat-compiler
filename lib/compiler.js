...
class GlobalContext {
  ...
  lookup (name) {
    let index = this.funcs.map(x => x.name).indexOf(name)
    if (!~index) index = this.globals.map(x => x.name).indexOf(name)
    if (!~index) index = this.types.map(x => x.name).indexOf(name)
    return index
  }
}
...
export default function compile (node) {
  ...
  const cast = param => param.kind === 'number' ? parseFloat(param.value) : param.value
  ...
  class FunctionContext {
    ...
    lookup (name) {
      let index = this.locals.indexOf(name)
      if (!~index) {
        index = this.blocks.indexOf(name)
        if (~index) index = this.blocks.length - 1 - index // TODO: need block depth?
      }
  ...
  function evaluate (node, context = g) {
    ...
    switch (instr) {
      ...
      case 'i32.load': {
        // TODO: handle offset/align properly (add test case)
        const args = [2,0]
        const expr = node.children.flatMap(x => evaluate(x, context))
        return bytes(instr, args, expr)
      }
      ...
      case 'block': {
        const name = node.name?.value ?? context.blocks.length
        const results = []
        const body = []

        context.blocks.push(name)
