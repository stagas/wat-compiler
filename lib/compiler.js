...
export default function compile(node, moduleData, globalData) {
  ...
  function evaluate(node, context = g) {
    ...
    switch (instr) {
      ...
      case 'call_indirect': {
        ...
        const expr = node.children.flatMap(function evaluateExpr(x) { return evaluate(x, context) })
        return bytes(instr, args, expr)
      }

      case 'memory.grow': {
        const args = [0] // TODO: this bit is reserved?
        const expr = node.children.flatMap(function evaluateMemory(x) { return evaluate(x, context) })
        return bytes(instr, args, expr)
      }
      ...
      case 'i64.store32':

        {
          ...
          const expr = node.children.flatMap(function evaluateLoadStoreExpr(x) { return evaluate(x, context) })
          return bytes(instr, args, expr)
        }
      ...
      case 'result': {
        return node.children.flatMap(function evaluateResult(x) { return INSTR.type[x.instr.value]() })
      }

      case 'else':
      case 'then': {
        return node.children.flatMap(function evaluateElseThen(x) { return evaluate(x, context) })
      }
      ...
      case 'block': {
        ...
        return [
          ...INSTR[instr](),
          ...results.flat().map(function layoutResults(x) { return [...x] }),
          ...body.flat(),
          ...INSTR.end()
        ]
      }
      ...
      case 'br_table': {
        ...
        const expr = node.children.flatMap(function evaluateBrTable(x) { return evaluate(x, context) })
      ...
      default: {
        ...
        const args = node.params.map(x => cast(x.param, context, instr))
        const expr = node.children.flatMap(function evaluateNode(x) { return evaluate(x, context) })
        return bytes(instr, args, expr)
      }
    }
  }
  ...
  function build(node) {
    switch (node.instr.value) {
      case 'module': {
        node.children.forEach(function buildModule(x) { build(x) })
      }
        break
      ...
      case 'func': {
        ...
        deferred.push(function deferredFunc() {
          m.func(
            ...
            func.results,
            func.locals,
            [...func.body.flatMap(function evaluateFuncBody(x) { return evaluate(x, func.context) })]
          )
        })
      }
        break
  ...
  deferred.forEach(function buildFunc(fn) { fn() })
