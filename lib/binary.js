...
section.table = function (tables) {
  return section('table',
    vector(tables.map(([type, size]) =>
      [BYTE.type[type], ...limits(size)]
    )))
}
...
section.element = function (elements) {
  return section('element',
    vector(elements.map(([table_idx, offset_idx, funcs]) =>
      [...byte(table_idx), ...INSTR.i32.const(offset_idx), BYTE.end, ...vector(funcs)]
    )))
}
